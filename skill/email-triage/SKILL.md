...
6. Produce the report defined in [references/report.md](references/report.md), including its short completion receipt when there are no findings to report. Advance cursors only after every page in the frozen window was classified and the report or receipt is ready; never advance a cursor from a partial or failed backlog run. Persist processed fingerprints, candidates, and a cursor only for accounts that completed successfully. Confirm successful state recording before sending a completion receipt. Never persist bodies, attachments, codes, or tokens.
